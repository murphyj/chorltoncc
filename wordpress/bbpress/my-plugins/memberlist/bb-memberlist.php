...
Change Log: .73a - Added Active User Count on memberlist page
			...
			.80  - Made the plugin compatible with themes
				 - Plugin now deletes users from the database (instead of changing their status)
...
function bb_mlist_delete_users() {
	...
	foreach ( $tode as $d ) :
		if ( $bb->wp_table_prefix )
			...
			$delteeded = $bbdb->query("DELETE FROM $bbdb->users WHERE ID = $d"); 
	endforeach; }
}
